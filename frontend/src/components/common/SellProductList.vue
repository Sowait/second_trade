    ...
    <div class="bg-white rounded-xl shadow-card overflow-hidden">
      ...
      <div class="p-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          ...
          >
            <div class="relative cursor-pointer" @click="$emit('view', p.id)">
              <img :src="p.image" :alt="p.title" class="w-full h-48 object-cover" />

              <div
                v-if="p.status === 'pending_review'"
                class="absolute top-2 left-2 bg-primary/90 text-white text-xs px-2 py-1 rounded"
              >
                待审核
              </div>
              <div
                v-else-if="p.status === 'rejected'"
                class="absolute top-2 left-2 bg-danger/90 text-white text-xs px-2 py-1 rounded"
              ...
              >
                <div class="flex space-x-2">
                  <button
                    class="bg-white text-neutral-700 w-12 h-12 rounded-full hover:bg-neutral-100 transition-colors flex items-center justify-center"
                    title="查看详情"
                    type="button"
                  ...
                  <button
                    ...
                    class="bg-white text-neutral-700 w-11 h-11 rounded-full hover:bg-neutral-100 transition-colors flex items-center justify-center"
                    title="编辑商品"
                    type="button"
            ...
            <div class="p-3">
              ...
              <p v-if="p.status === 'rejected' && p.rejectReason" class="text-xs text-danger mt-1 line-clamp-2">
                驳回原因：{{ p.rejectReason }}
              </p>
            </div>
          </div>
...
function mapBackendProductToItem(p: any): ProductItem {
  ...
  const status = p?.status != null ? String(p.status) : (p?.product_status != null ? String(p.product_status) : "");
  const rejectReason =
    p?.review_reject_reason != null ? String(p.review_reject_reason) : (p?.reject_reason != null ? String(p.reject_reason) : "");
...
async function fetchProducts(page = 1) {
  ...
  try {
    ...
    const onSaleCount = list.filter((x: any) => String(x?.status ?? "") === "on_sale").length;
    const viewCount = list
      .filter((x: any) => x?.view_count != null)
      .reduce((acc: number, x: any) => acc + toNum((x as any).view_count), 0);
