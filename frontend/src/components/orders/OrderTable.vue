            ...
            <div class="md:hidden">
              ...
              <div class="grid grid-cols-2 gap-y-2 gap-x-4 w-full mt-4">
                ...
                <div class="col-span-2 flex justify-end flex-wrap gap-2 mt-2">
                  <button
                    v-for="btn in getActionButtons(o)"
                    :key="btn.action"
...
function onPageNumberClick(n: { type: "page" | "ellipsis"; page?: number }) {
  if (n.type !== "page") return;
  if (typeof n.page !== "number") return;
  emit("page", n.page);
}
...
function getOrderStatusText(o: OrderItem) {
  if (o.statusView) return o.statusView;
  if (o.pickup && o.status === "pending_shipment") return "待自提";
  return getStatusText(o.status);
}

function getActionButtons(o: OrderItem) {
  ...
  if (props.tab === "bought") {
    ...
    if (o.pickup && o.status === "pending_shipment") {
      buttons.push({ text: "确认自提", className: primary, action: "confirm" });
      buttons.push({ text: "申请退款", className: outlineDanger, action: "refund" });
    } else if (o.status === "pending_receipt" || o.status === "shipped") {
