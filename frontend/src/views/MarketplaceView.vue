...
async function fetchMarketProducts(page?: number) {
  loading.value = true;
  errorMsg.value = null;
  ...
  try {
    ...
    const rawTotal = Array.isArray(data) ? list.length : Number(data?.count ?? data?.total);
    const safeTotal = Number.isFinite(rawTotal) && rawTotal >= 0 ? rawTotal : list.length;
    totalCount.value = Math.max(safeTotal, list.length);
  } catch (e: any) {
    products.value = [];
    totalCount.value = 0;
...
watch(
  filters,
  () => {
    if (enteredCategoryId.value) {
      currentPage.value = 1;
      fetchMarketProducts(1);
    }
  },
  { deep: true }
);
