  ...
  <main class="flex-grow container mx-auto px-4 py-8">
    <div id="order-center-page" class="space-y-6">
      ...
      <OrderDetailModal
        :open="detailOpen"
        :order="detailOrder"
        :actions="getActionButtons(detailOrder)"
      ...
      <div v-if="payConfirmOpen" class="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div class="relative w-[92vw] max-w-md bg-white rounded-xl shadow-lg border border-neutral-200 p-6">
          ...
          <label class="mt-4 flex items-center gap-2 text-neutral-700 select-none">
            <input v-model="payPickup" type="checkbox" class="h-4 w-4 rounded border-neutral-300" />
            <span>自提</span>
          </label>
...
async function confirmPay() {
  ...
  try {
    await payOrder(payTargetOrder.value.id, { pickup: payPickup.value });
    closePayConfirm();
    await loadOrders();
  } catch (e: any) {
...
function normalizeOrder(o: any): OrderItem {
  ...
  const pickup = Boolean(
    o?.pickup ??
      ...
      o?.self_pickup ??
      o?.pickup_enabled ??
      String(o?.delivery_method ?? "") === "pickup"
  );
  const status = (o?.status ?? "") as Status;
  const statusView = o?.status_view
    ? String(o.status_view)
    : pickup && status === "pending_shipment"
      ? "待自提"
      : statusToText(status);
  ...
  return {
    ...
    status,
    statusView: statusView as string,
    pickup,
...
function getActionButtons(order?: OrderItem | null) {
  const buttons: { text: string; className: string; action: string }[] = [];

  if (!order?.status) return buttons;

  if (tab.value === "bought") {
    // buyer actions

    if (order.pickup && order.status === "pending_shipment") {
      buttons.push({ text: "确认自提", className: "btn-primary", action: "confirm" });
      buttons.push({ text: "取消订单", className: "btn-secondary", action: "refund" });
    } else if (order.status === "pending_receipt" || order.status === "shipped") {
